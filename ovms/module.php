...
require_once __DIR__ . '/../libs/VariableProfileHelper.php';
	...
	class OVMS extends IPSModule
	{
		...
		public function ReceiveData($JSONString)
		{
			...
			{
				//$deviceTopic	= $this->ReadPropertyString("MQTTTopic");				
				$deviceTopic = $this->ReadAttributeString("MQTTTopic");
				$topicMetric = str_replace($deviceTopic."metric/", "", $data['Topic']);
				$IdentName = str_replace("/","_",$topicMetric);				
				$DBFound = "no";
				...
				foreach ($DP as $DataPoint){
					if (in_array($topicMetric, $DataPoint)) {
						...
						$DBFound = "yes";
						$this->SendDebug("Datapoint found in Database",$topicMetric, 0);
					}
				}

				if ($DBFound === "no")
				{
					$this->SendDebug("Datapoint not found in Database",$topicMetric, 0);
